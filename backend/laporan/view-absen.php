...
?>
<h2>LAPORAN ABSENSI PER MATA PELAJARAN</h2>
  ...
  <tr>
    ...
    <td> <select name="kelas" style="width: 70px;">
                ...
                <?php
                                ...
                                $ambil=mysql_query("select * from kelas");
                                while($r=mysql_fetch_array($ambil)){
...
<table class="table table-striped table-bordered table-hover" id="dataTables-example">
 ...
 if(isset($_POST['btnTampil'])) {
	...
	$SqlPeriode = " TANGGAL BETWEEN '".InggrisTgl($tglAwal)."' AND '".InggrisTgl($tglAkhir)."'";
 	...
 	$tampil=  mysql_query("SELECT absensi.NIS,absensi.KETERANGAN,absensi.TANGGAL,siswa.NAMA_SISWA,
                    ...
                    AND absensi.SEMESTER = '$Semester'
                    AND absensi.THAjar = '$THN_AJAR'
                    AND
 ...
 ?>
  <tr>
     <td colspan="5"><a href="laporan/lap_absen_mapel.php?mp=<?=$mp?>&Kelas=<?php echo $_POST['kelas'];?>&Tawal=<?php echo $tglAwal;?>&Takhir=<?php echo $tglAkhir; ?>&print=Y" class="btn btn-warning" >Cetak</a> <a href="laporan/lap_absen_mapel.php?mp=<?=$mp?>&Kelas=<?php echo $_POST['kelas'];?>&Tawal=<?php echo $tglAwal;?>&Takhir=<?php echo $tglAkhir; ?>&print=N"  class="btn btn-warning" >Excel</a></td>
 </tr>
</table>
